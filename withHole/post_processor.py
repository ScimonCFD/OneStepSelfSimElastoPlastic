...
for load_inc in range(1, endTime+1):  
    ...
    ep_nn_epsilon_field = np.concatenate((ep_nn_epsilon_field, ep_eps_Right,
                                          ep_eps_Down, ep_eps_Up, ep_eps_Hole), 
                                          axis = 0)

    ep_nn_sigma_field = np.concatenate((ep_nn_sigma_field, ep_sig_Right,
                                        ep_sig_Down, ep_sig_Up, ep_sig_Hole), 
                                        axis = 0)
...
for com in range(len(component)):            
    # if (com in (0, 1, 3)):
        if (com in dum_ind):
            ...
            fig = plt.figure(figsize=(15, 10))
                # if (ML_MODEL_IS_3X3):
            plt.plot(theoretical_ep_epsilon_field[-1, :, :][:600, com], 
                     theoretical_ep_epsilon_field[-1, :, :][:600, com], 
                     color = "blue", label = "Expected", marker='x')                
            plt.scatter(theoretical_ep_epsilon_field[-1, :, :][:600, com], 
                        elastic_nn_epsilon_field[-1, :, :][:600, com], 
                        color = "red", label = "ElasticNN", marker='o', 
                        alpha = 0.3)                
            plt.scatter(theoretical_ep_epsilon_field[-1, :, :][:600, com], 
                        theoretical_elastic_epsilon_field[-1, :, :][:600, com], 
                        color = "purple", label = "ElasticTheoretical", 
                        marker='+')   
            plt.scatter(theoretical_ep_epsilon_field[-1, :, :][:600, com], 
                        ep_nn_epsilon_field[-1, :, :][:600, com], 
                        color = "green", label = "Calculated", marker='o', 
                        alpha=(0.3))          
...
for com in range(len(component)):            
    # if (com in (0, 1, 3)):
        if (com in dum_ind):
            ...
            plt.plot(theoretical_ep_sigma_field[-1, :, :][:600, com], 
                    theoretical_ep_sigma_field[-1, :, :][:600, com], 
                    color = "blue", label = "Expected", marker='x')                
            plt.scatter(theoretical_ep_sigma_field[-1, :, :][:600, com], 
                        elastic_nn_sigma_field[-1, :, :][:600, com], 
                        color = "red", label = "ElasticNN", marker='o', 
                        alpha = 0.3)                
            plt.scatter(theoretical_ep_sigma_field[-1, :, :][:600, com], 
                        theoretical_elastic_sigma_field[-1, :, :][:600, com],
                        color = "purple", label = "ElasticTheoretical",
                        marker='+')   
            plt.scatter(theoretical_ep_sigma_field[-1, :, :][:600, com],
                        ep_nn_sigma_field[-1, :, :][:600, com], color = "green", 
                        label = "Calculated", marker='o', alpha=(0.3))  
            plt.xlabel(r'Expected stress $(Pa)$')
            plt.ylabel(r'Calculated stress $(Pa)$')
            # fig.suptitle("Sigma_" + component[com], fontsize=20)
...
D_expected_hole = deserialise(ROUTE_THEORETICAL_MODEL +  str(int(load_inc)) + 
                                          "/", "D_Hole")
D_ep_nn_hole = deserialise(ROUTE_EP_NN_MODEL + str(int(load_inc)) + "/", 
                          "D_Hole")

D_elastic_expected_hole = deserialise(ROUTE_THEORETICAL_ELASTIC_MODEL + 
                                    str(int(load_inc)) + "/", "D_Hole")

D_elastic_nn_hole = deserialise(ROUTE_ELASTIC_NN_MODEL + str(int(load_inc)) + 
                                 "/", "D_Hole")
